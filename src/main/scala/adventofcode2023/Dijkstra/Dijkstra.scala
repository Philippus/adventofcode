package adventofcode2023.Dijkstra

import scala.annotation.tailrec
import scala.collection.mutable

/** Algorithm copied from https://github.com/novakov-alexey-zz/scala-dijkstra
  *
...
object EdgeWeightedDigraphOps {
  ...
  implicit class EdgeWeightedDigraphOps(g: EdgeWeightedDigraph) {
    ...
    /** Adds directed edge 'e' to EdgeWeightedDigraph by creating a full-copy adjacency list, i.e. this operation leaves
      * current graph 'g' immutable
      *
      * @param e
      *   \- DirectedEdge to add into the graph 'g'
      * @return
      *   returns new graph with edge 'e' added to it
      */
    ...
    def addEdges(s: Seq[DirectedEdge]): EdgeWeightedDigraph = {
      var adj = g.adj
      s.foreach { e =>
        val list = adj.getOrElse(e.from, List.empty)
        adj = adj + (e.from -> (list :+ e))
      }
      EdgeWeightedDigraph(adj)
    }
  }
...
object ShortestPath {
  ...
  /** Function tries to find a shortest path from source vertex to all other vertices in the graph
    *
    * @param g
    *   EdgeWeightedDigraph to find a shortest path
    * @param sourceV
    *   source vertex to find a shortest path from
    * @return
    *   return either error as string when input parameters are invalid or return shortest path result
    */
...
/** @param edgeTo
  *   a sequence which represents the last edge on the shortest path from 'sourceV' to vertex i. None means there is no
  *   path to vertex i
  * @param distTo
  *   a sequence of distances from source vertex to a specific i vertex
  */
class ShortestPathCalc(edgeTo: Seq[Option[DirectedEdge]], distTo: Seq[Double]) {

  /** @param v
    *   vertex to get the path for
    * @return
    *   returns error when v is invalid or sequence of edges which form the path from source vertex to v vertex
    */
  ...
  /** @param v
    *   vertex to check whether path from source vertex to v vertex exists
    * @return
    *   returns error when v is invalid or Boolean when some path from source vertex to vertex v exists
    */
  def hasPath(v: Int): Either[String, Boolean] =
    distTo.lift(v).map(_ < Double.PositiveInfinity).toRight(s"Vertex $v does not exist")

  /** @param v
    *   vertex to get the distance for
    * @return
    *   returns error when v is invalid or the Double distance which is a sum of weights
    */
