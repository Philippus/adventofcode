...
object Day24:
  def determineMinimumCombinations(packages: Seq[BigInt]): BigInt =
    ...
    (for
      combination <- packages.combinations(14).filter(_.sum == weight) // determined that the biggest group contains 14 packages
      j <- 1 to 14
      combinationTwo <- packages.diff(combination).combinations(j).filter(_.sum == weight)
      combinationThree = packages.diff(combination).diff(combinationTwo) // last group's weight doesn't need to be checked
  ...
  def determineMinimumCombinationsForPartTwo(packages: Seq[BigInt]): BigInt =
    ...
    (for
      combination      <- packages.combinations(13).filter(_.sum == weight) // determined that the biggest group contains 13 packages
      j                <- 1 to 13
      combinationTwo   <- packages.diff(combination).combinations(j).filter(_.sum == weight)
      k                <- 1 to 13
      combinationThree <- packages.diff(combination).diff(combinationTwo).combinations(k).filter(_.sum == weight)
      combinationFour   = packages.diff(combination).diff(combinationTwo).diff(combinationThree)  // last group's weight doesn't need to be checked
