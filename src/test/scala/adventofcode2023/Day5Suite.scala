  ...
  test("part 1 works for the sample input"):
    def readInputDocument: Long =
      Using.resource(Source.fromResource("day5sampleinput.txt")): source =>
        ...
        val (_, humiditiyToLocation)   = lines.splitAt(lines.indexOf("humidity-to-location map:"))
        val humiditiyToLocationMap     = generateMap(humiditiyToLocation.tail)
        seeds.map(seed =>
          seedToLocation(
            Seq(
              seedToSoilMap,
